...
Speed2QPSS = 2000.0/(0.127*math.pi)
...
def callback(data):
    ...
    wheel_velocity_cmd_lift = float(data.linear.x -(data.angular.z*wheel_separation*0.5))
    global wheel_velocity_cmd_right 
    wheel_velocity_cmd_right = float(data.linear.x +(data.angular.z*wheel_separation*0.5))
...
def cmd_vel_move():
    ...
    while not rospy.is_shutdown():
        ...
        if math.isnan(wheel_r):
            wheel_r = 0.0
        #drive Motor
        vl = int(wheel_velocity_cmd_lift*Speed2QPSS)
        vr = int(wheel_velocity_cmd_right*Speed2QPSS)
        
	rc.SpeedM1M2(0x80,vl,vr)
        ...
        rospy.loginfo(' wheel_L/sec :' + str(wheel_l*10.0) +' wheel_R/sec :'   + str(wheel_r*10.0) )
